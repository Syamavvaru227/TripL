    ...
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-[100] glass border-t border-border">
      <div className="flex items-center justify-around h-16 px-1">
        {tabs.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <Link key={to} to={to} className={`flex min-w-0 flex-1 flex-col items-center gap-0.5 px-1 py-1 rounded-xl transition-all duration-150 ${active ? "text-saffron" : "text-muted"}`}>
